...
const FSM = {
    ...
    await chat.sendStateTyping();
    await delay(1500);
    await client.sendMessage(msg.from, 'Certo, obrigado pelas informações! Em breve um de nossos atendentes irá te responder. 🕐');
    ...
    'aguardando_dados': async (msg, chat) => {
        ...
        await chat.sendStateTyping();
        await delay(1500);
        await client.sendMessage(msg.from, `Certo, obrigada pelas informações!\nDigite "encerrar" para iniciar uma nova interação.`);
        timeoutHandler(chat, 30 * 60 * 1000, msg.from);
    }
};
...
async function handleAlunoMenu(msg, chat) {
    const introductions = {
        '1': '> Código atendimento: RQM01#\n\n🔎 Vamos iniciar a verificação do seu requerimento.\n\nPor favor, envie os seguintes dados para que possamos verificar.',
        '2': '> Código atendimento: FNC02#\n\n💰 Vamos tratar das questões financeiras.\n\nPor favor, envie os seguintes dados para que possamos verificar.',
        ...
        '10': '📢 Canal da Ouvidoria da instituição.',
    };
    ...
    const responses = {
        ...
        '30': '> Histórico escolar:\n\nAcessar o Portal do Aluno: https://sistemas.portaledu.com.br/FrameHTML/web/app/edu/PortalEducacional/login/\n>> Ir em "Secretaria", na opção "Requerimentos"\n>> Selecionar a opção "EDU - Histórico Escolar 1ª Via" (caso seja sua 1ª solicitaçaõ desse documento),\nou a opção "EDU - Histórico Escolar 2ª Via" (caso seja sua 2ª ou + solicitação desse requerimento, *lembrando a 2ª Via possui uma taxa de emissão no valor de R$30,00*. \n\n Ciente de que a solicitação repetida da 1ª Via pode ser indeferida, pois a 1ª Via pode-se solicitar apenas na primeira vez. Uma vez solicitada a 1ª Via, as próximas somente serão emitidas via pagamento da taxa.',
        '4': '> Justificativa de falta\n\nA documentação (declaração ou atestado) deve ser entregue na secretaria presencialmente.\nA entrega é feita mediante *pagamento da taxa de R$30,00* para abertura do requerimento de justificativa de falta.',
        ...
        '6': '> Código atendimento: EMPRE06#\n\n💼 Comunidade Empremetro\n\nTodos os dias publicamos vagas de estágio e emprego - não só de empresas parceiras, como também de nossa instituição - para que você, nosso(a) prezado(a) aluno(a), possa se candidatar com mais facilidade e agilidade. Fique ligado(a) nas oportunidades! 😉\nCaso não faça parte da comunidade, envie seu nome completo e contato atualizado para incluírmos você em nossa comunidade\n\nCaso queira se cadastrar no banco de talento da Fametro Leste, acesse nosso formulário e preencha suas informações: https://forms.gle/TL6DB3TcBGkJY4rj7',
        '7': '> Formulário de cadastro\n\nhttps://forms.gle/gBFTa5dw94ud1ToD7',
        '8': '> Código atendimento: ATCE08#\n\nPor favor, compareça com suas vias presencialmente para assinatura no setor NADI/CRA (em frente à coordenação).\n\nImportante verificar:\n• Se há mais de uma via, para que uma via fique na instituição.\n• Se todas as vias estão assinadas por você (discente).\n• Se as informações da instiuição estão corretas na documentação, principalmente um nome de um *responsável* da instuição.\n\nCaso tenha sido enviado via e-mail, informe seu nome completo e nº de matrícula para verificarmos.',
        '9': 'Prezado(a) aluno(a),\n\nSolicitações de cancelamento/trancamento devem ser feitas presencialmente no setor NADI/CRA (em frente à sala de coordenação) mediante pagamento de taxa no valor de *R$30,00*.\n\n *Obs.*: Informações complementares à solicitação de cancelamento e trancamento podem ser encontradas na Cláusula Oitava e Cláusula Nona de seu contrato de matrícula.',
        '10': 'Acesse o link para realizar uma ouvidoria: https://fametro.edu.br/ouvidoria/',
    };
    ...
    if (msg.body === '1') {
        ...
        setUserState(msg.from, 'esperando_dados_requerimento'); // Aguarda dados do aluno
        timeoutHandler(chat, 30 * 60 * 1000, msg.from);
    } else if (['5', '7', '9', '10'].includes(msg.body)) {
        await chat.sendStateTyping();
        await delay(1500);
...
async function handleNaoAlunoMenu(msg, chat) {
    ...
    const responses = {
        '1': 'Detalhes dos cursos... (edite conforme necessário)',
        '2': 'Formas de ingresso... (edite conforme necessário)',
        '3': 'Mensagem da ouvidoria... (edite conforme necessário)',
    };
    ...
    if (responses[msg.body]) {
        ...
        await client.sendMessage(msg.from, 'Opção inválida. Por favor, digite um número de 1 a 4 ou "encerrar" para finalizar.');
    }
}
